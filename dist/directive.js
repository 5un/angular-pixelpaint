...
angular.module('angularPixelPaint', []).directive('pixelPaint', ['$document', '$window', '$q', '$timeout', '$log', function($document, $window, $q, $timeout, $log) {
    ...
    function linkFunc(scope, el) {
        ...
        var getMinScreenBox = function() {
            return Math.floor(Math.min(el[0].offsetWidth, el[0].offsetHeight));
        };
        var convertRatioToCellSize = function(ratioCell) {
          return Math.floor(getMinScreenBox() / ratioCell);
        };
        var convertCellSizeToRatio = function(cs) {
          return Math.floor(getMinScreenBox() / cs);
        };
        // var expectDefaultRatioCell = 15;
        // var maxRatioCell = 20;
        // var minRatioCell = 10;
        // var rangeRatioCell = (maxRatioCell - minRatioCell) / 2;
        // var setDefaultCellSize = function() {
        //     if ((convertCellSizeToRatio(scope.options.cellSize) - expectDefaultRatioCell) > (rangeRatioCell / 2 + 1)) {
        //         scope.options.cellSize = convertRatioToCellSize(expectDefaultRatioCell);
        //     }
        // };
        // setDefaultCellSize();
        //
        // var overRangeRatioCell = function(ratioCell) {
        //     return ratioCell < minRatioCell || ratioCell > maxRatioCell;
        // };
        var overScreenX = function(x, w) {
          return x < 0;// || (x + w > el[0].offsetWidth;
        };

        var overScreenY = function(y, h) {
          $log.info('================');
          $log.info(el[0].offsetHeight);
          $log.info("y = " + y);
          $log.info("h = " + h);
          $log.info("y + h = " + (y + h));
          $log.info('================');
          return y < 0;// || (y + h) > el[0].offsetHeight;
        };
        ...
        var layers = [],
            options = {
                cellSize: 10,
                // cellSize: 10convertRatioToCellSize(expectDefaultRatioCell),
                largeGridEvery: 10,
                gridLineWidth: 1,
        ...
        optionsWatcher = scope.$watch('options', function(newValue, oldValue) {
            ...
            //if (newValue.cellSize > 0 && !overRangeRatioCell(convertCellSizeToRatio(newValue.cellSize))) {
        ...
        var onPanMove = function(ev) {
            if (selectedTool == 'move') {
                ...
                var panOffX = panStartLayerOffset.x * options.cellSize;
                var panOffY = panStartLayerOffset.y * options.cellSize;
                ...
                var w = options.imageWidth * options.cellSize;
                var h = options.imageHeight * options.cellSize;
                var overX = overScreenX(absPos(delta.x), w);
                var overY = overScreenX(absPos(delta.y), h);
                ...
                if (!overX && !overY) {
                    ...
                    panMoveAllOffset.x = Math.floor(delta.x / options.cellSize);
                    el[0].querySelector('.layers-container').style.transform = 'translate(' + absPos(delta.x) + 'px,' + absPos(0) + 'px)';
                    //el[0].querySelector('.layers-container').style.transform = 'translateX(' + absPos(delta.x) + 'px)';
                }
                else if (!overY) {
                    panMoveAllOffset.y = Math.floor(delta.y / options.cellSize);
                    el[0].querySelector('.layers-container').style.transform = 'translate(' + absPos(0) + 'px,' + absPos(delta.y) + 'px)';
                }
            } else {
                var activeLayer = getActiveLayer();
